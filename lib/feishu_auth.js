...
//   GET  /auth/feishu/fbif/sso-config    FBIF 飞书 webview 免登配置（返回 appID）
//   POST /auth/feishu/fbif/sso-exchange  FBIF 飞书 webview 免登 code 交换（v2 token，不带 redirect_uri）
//   GET  /auth/feishu/fude/*             富的 同上 6 条
//   POST /auth/feishu/logout             退出登录（共享）
//
// 环境变量：
//   FEISHU_FBIF_APP_ID / FEISHU_FBIF_APP_SECRET    FBIF App
//   FEISHU_FUDE_APP_ID / FEISHU_FUDE_APP_SECRET    富的 App
...
function tenantConfig(tenant) {
  ...
  if (tenant === "fude") {
    return {
      appId: (process.env.FEISHU_FUDE_APP_ID || "").trim(),
      appSecret: (process.env.FEISHU_FUDE_APP_SECRET || "").trim(),
    };
  }
...
function signingSecret() {
  ...
  const fude = (process.env.FEISHU_FUDE_APP_SECRET || "").trim();
